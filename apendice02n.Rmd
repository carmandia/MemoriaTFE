  ...
  for (b in Cartas) {
    ...
    for (j in 1:10) {
      ...
      G_continuar <- G_continuar + 
        Probabilidades_sacar_carta[j] * ganancia_G_optima[as.character(nueva_z),b] 
    }
    if (2*G_continuar > G_estrella) {
...
for (b in Cartas) {
  ...
  for (j in 1:10) {
    ...
    G_continuar <- G_continuar + 
      Probabilidades_sacar_carta[j] * Calculo_G_0(nueva_z,b,tabla = resultados_tabla)
  }
  if (2*G_continuar > G_estrella) {
...
Todo el código completo se puede encontrar en mi GitHub personal que se encuentra referenciado en la bibliografía [@Libro21].